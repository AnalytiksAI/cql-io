...
setAuthentication =
    set (connSettings . authenticators)
        . HashMap.fromList
...
defRetryHandlers =
    [ const $
        Handler $ \(e :: ConnectionError) -> case e of
            ConnectTimeout {} -> return True
            _ -> return False
    , const $
        Handler $ \(e :: ResponseError) -> return $ case reCause e of
            Unavailable {} -> True
            ReadTimeout {..} ->
                rTimeoutNumAck >= rTimeoutNumRequired
...
eagerRetryHandlers =
    [ const $
        Handler $ \(e :: ResponseError) -> case reCause e of
...
setRetryHandlers ::
    (forall m. Monad m => [RetryStatus -> Handler m Bool]) ->
    RetrySettings ->
    RetrySettings
